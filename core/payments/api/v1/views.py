    ...
    try:
       ...
       try:
           ...
           transaction = Transaction.objects.create(
               user=user, description=f"Payment for Order #{order.id} - Total Price: {amount}",
               authority=bank_record.tracking_code, status="pending"
           )
           Payment.objects.create(order=order, transaction=transaction, is_successful=False)
